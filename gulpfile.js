...
var serviceList = [
  {
    url: 'http://120.77.56.94:8100/',// 域名生产环境 0
    loginInfo: {
      username: 'admin',
      password: 'admin'
    }
  ...
  {
    url: 'http://test.coracle.com:10070/',// 域名生产环境 0
    loginInfo: {
      username: 'admin',
      password: 'admin'
    }
...
gulp.task('copy',  function() {
  return gulp.src('./src/public/img/**').pipe(gulp.dest(config.dest+'/public/img'));
});
/*
 ====================================
 =            zip Task            =
 ====================================*/
gulp.task('zip', ['clean_dist', 'copy'], function (done) {
  for (var key in baseWebpackConfig.entry) {
    gulp.src(['!**/*.map', './' + config.dest + '/' + key + '/**'], {base: config.dest})
        .pipe(zip(key + '.zip'))
        .pipe(gulp.dest('./zip'));
  }
  gulp.src(['!**/*.map', './' + config.dest + '/register/**', './' + config.dest + '/public/**'], {base: config.dest})
    .pipe(zip('register.zip'))
    .pipe(gulp.dest('./zip'));
