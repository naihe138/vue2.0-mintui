...
function iosAgentRequest (rqStr) {
  setTimeout(function () {
    // 动态创建框架
...
function ajax (option) {
  ...
  option.emulateJSON = typeof (option.emulateJSON) == 'undefined' ? false : option.emulateJSON;
  ...
  var postData = {
    'header': option.header || {
      'content-Type': option.emulateJSON ? 'application/x-www-form-urlencoded' : 'application/json'
    },
    'reqURL': option.url,
    'reqType': option.type,
    'sCallback': funcName,
  ...
  if (os.ios && !flag) {
    iosAgentRequest('reqInterfaceProxy(' + JSON.stringify(postData) + ')');
    /*
     setTimeout(function () {
    ...
    if (jsonToUrl) {
      str += '?';
      // 接口传参：一部分用post请求，然后拼接参数到url后面（类似与get）；一部分用post请求，拼接一个jsonparm
      if (option.isJsonParam) {
        str += 'jsonparm=' + encodeURI(JSON.stringify(option.param));
      } else {
        for (key in option.param) {
          str += key + '=' + option.param[key] + '&';
        }
...
function getQDLocationInfo (param) {
  ...
  jsonParam = JSON.stringify(jsonParam);
  if (os.ios) {
    iosAgentRequest('getQDLocationInfo(' + jsonParam + ')');
    ...
    /*
     ...
     var iframe = document.createElement('iframe');
     // 框架中加载的页面
     iframe.src = 'getQDLocationInfo(' + jsonParam + ')';
     iframe.style.display = 'none';
     document.body.appendChild(iframe);
     }, 50);
     */
 ...
 * 初始化SDK
 */
function initYzkSdk (param) {
  window['initYzkSdk'] = function (result) {
    param.success(result);
  };
  var jsonParam = param || {};
  jsonParam.sCallback = 'initYzkSdk';
  //1 单次扫描,2连续扫描
  jsonParam = JSON.stringify(jsonParam);
  if (os.ios) {
    iosAgentRequest('goYunZK(' + jsonParam + ')');
    /*
     setTimeout(function () {
     // 动态创建框架
     var iframe = document.createElement('iframe');
     // 框架中加载的页面
     iframe.src = 'goYunZK(' + jsonParam + ')';
     iframe.style.display = 'none';
     document.body.appendChild(iframe);
     }, 50);
     */
    ...
    jsonParam.data = JSON.parse(jsonParam.data);
    jsonParam= JSON.stringify(jsonParam);
    kndfunc.blnNetWorkF1(jsonParam);
  } else {
    console.log('初始化SDK' + jsonParam);
    param.success({code: -1});
  }
}
...
var loading = {
  ...
  show: function (title, locked) {
    ...
    if (loadingDom.style.display != 'block') {
      if (title != '') {
        title += '&nbsp;&nbsp;&nbsp;&nbsp;'
      }
...
var loginOut = function (data) {
  if (os.ios) {
    window.location.href = 'loginOut()';
  } else if (os.android) {
    kndfunc.loginOut(JSON.stringify(data));
    console.log('退出登录');
  } else {
    console.log('退出登录');
  }
}
var getAppInfo = function (param) {
  window['getAppInfo'] = function (result) {
    param.success(result);
  };
  var jsonParam = {sCallback: 'getAppInfo'};
  if (os.ios) {
    window.location.href = 'getAppInfo(' + JSON.stringify(jsonParam) + ')';
  } else if (os.android) {
    kndfunc.getAppInfo(JSON.stringify(jsonParam));
  } else {
    console.log('获取APP相关信息');
  }
}

var checkUpdate = function (param) {
  window['checkUpdate'] = function (result) {
    param.success(result);
  };
  var jsonParam = {sCallback: 'checkUpdate'};
  if (os.ios) {
    window.location.href = 'checkUpdate(' + JSON.stringify(jsonParam) + ')';
  } else if (os.android) {
    kndfunc.checkUpdate(JSON.stringify(jsonParam));
  } else {
    console.log('获取升级信息');
  }
};
// 返回登录的页面
var gobackLogin = function (param) {
  window['toLoginPage'] = function (result) {
    if (typeof result === 'string') {
      result = JSON.parse(result);
    }
    param.success(result);
    window['toLoginPage'] = null;
  };
  var jsonParam = param.apiJson || {};
  jsonParam.sCallback = 'toLoginPage';
  if (os.android) {
    kndfunc.toLoginPage();
  } else if (os.iphone) {
    window.location.href = 'toLoginPage(' + JSON.stringify(jsonParam) + ')';
  } else {
    console.log('获取升级信息', os);
  }
};
// 注册成功后自动登录跳转到首页
var registerToLogin = function (param) {
  if (navigator.userAgent.match(/Android/g)) {
    kndfunc.registerToLogin(JSON.stringify(param));
  } else if (os.iphone) {
    window.location.href = 'registerToLogin('+ JSON.stringify(param) +')';
  } else {
    console.log('注册完成后自动登录，并跳转到首页');
  }
};
// 获取当前手机连接的WiFi名
var getCurrentWiFiSSID = function (param) {
  window['sGetCurrentWiFiSSID'] = function (result) {
    var _result = result
    param.success(_result.WiFiSSID);
  };
  param.sCallback = 'sGetCurrentWiFiSSID';
  param.fCallback = 'fGetCurrentWiFiSSID';
  window['fGetCurrentWiFiSSID'] = function () {
    // 如果没有获取到会调用此方法
    param.error();
  }
  if (navigator.userAgent.match(/Android/g)) {
    kndfunc.getCurrentWiFiSSID(JSON.stringify(param));
  } else if (os.iphone) {
    window.location.href = 'getCurrentWiFiSSID('+ JSON.stringify(param) +')';
  } else {
    console.log('初始化的时候获取当前手机连接的WiFi');
  }
};
